...
contract("IPFSInbox", accounts => {
  it("...should emit an event when you send an IPFS address.", 
    async () => { // Wait for the contract to be deployed
        const ipfsInbox = await IPFSInbox.deployed(); // Set a variable to false, and create an event listener
        ...
        await ipfsInbox.sendIPFS(accounts[1], 
            "SampleAddress", { from: accounts[0] }); // Check if the variable is set to true by this time
